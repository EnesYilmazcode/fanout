# Fanout Design Panel — Final Report
...
- `public/setup.html` (rename the "dashboard" — it is a setup page):
  ...
  - **Step 3 — Your config.** Tabs: env block / curl / Python / JS / "tools that can't set custom headers." The env block bundles the Fanout key plus comma-joined blobs, pre-assembled — the one-copyable-block moment.
...
4. **PR 4 — `feat(api): X-Fanout-Pool-Health header`** *(pre-existing P1)*. Surface which connections failed, not just which won. Directly serves the setup-page audience debugging a multi-provider pool.
